...
def plot_and_loss(eval_model, data_source, epoch):
    ...
    pyplot.title('Training process', pad=10)
    pyplot.plot(test_result,color="red", label='pred')
    pyplot.plot(truth[:500],color="blue", label='label')
    pyplot.plot(test_result-truth,color="green", label='diff')
...
def predict_future(eval_model, data_source, steps):
    ...
    with torch.no_grad():
        for i in range(0, steps):            
            inputs, target = get_batch(data_source, i , 1) # one-step forecast
    ...
    pyplot.title('Validation process', pad=10)
    pyplot.plot(test_result, color="red", label='pred') 
    pyplot.plot(truth, color="green", label='label')      
    # pyplot.plot(data[:input_window],color="blue", label='input')    
    pyplot.grid(True, which='both')
    pyplot.legend()
    pyplot.axhline(y=0, color='k')
...
def get_data():
    ...
    series = read_csv('sst.csv', header=0, parse_dates=True, squeeze=True).sst
...
plt.title('Model train process', pad=10)
plt.plot(np.arange(0,len(train_data[0][0])), 
         train_data[0][0].cpu().numpy(), label='input')
plt.plot(np.arange(1, len(train_data[0][0])+1), 
         train_data[0][1].cpu().numpy(),label='label', linestyle='--')
...
test_data = pd.read_csv('sst_test.csv', header=0, parse_dates=True, squeeze=True).sst

# looks like normalizing input values curtial for the model
scaler = MinMaxScaler(feature_range=(0, 1)) 
amplitude = scaler.fit_transform(test_data.to_numpy().reshape(-1, 1)).reshape(-1)

test_sequence = create_inout_sequences(amplitude, input_window ,output_window)

test = test_sequence.to(device)

# test data index input 
test_data = test[250]
test_input = test_data[0].reshape(30,1,1)
test_label = test_data[1]

test_out = model(test_input)

test_input = test_input.reshape(-1)
test_out = test_out.reshape(-1)


plt.title('Model test', pad=10)

plt.plot(np.arange(0,len(test_input)), 
         test_input.cpu().numpy(), label='input')

plt.plot(np.arange(1, len(test_input)+1), 
         test_label.cpu().numpy(), label='label', linestyle='--')

plt.plot(np.arange(1, len(test_input)+1), 
         test_out.detach().numpy(), label='pred', linestyle='--')
